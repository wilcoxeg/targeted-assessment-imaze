...
model_agg = model_df %>%
  mutate(gpt2 = (gpt2_1584550421 + gpt2_1585674487 + gpt2_1587139950) / 3,
         rnng = (rnng_38435 + rnng_62488 + rnng_7245) / 3) %>%
  rename(grnn = grnn_0,
         jrnn = jrnn_0) %>%
...
lm_gpt = d_model %>%
  filter(l_maze == T) %>%
  lmer(rt ~ gpt2 + len + freq + (1|MD5) + (1|group), data = .)
summary(lm_gpt)

lm_rnng = d_model %>%
  filter(l_maze == T) %>%
  lmer(rt ~ rnng + len + freq + (1|MD5) + (1|group), data = .)
summary(lm_rnng)

lm_grnn = d_model %>%
  filter(l_maze == T) %>%
  lmer(rt ~ grnn + len + freq + (1|MD5) + (1|group), data = .)
summary(lm_grnn)

lm_jrnn = d_model %>%
  filter(l_maze == T) %>%
  lmer(rt ~ jrnn + len + freq + (1|MD5) + (1|group), data = .)
summary(lm_jrnn)
...
get_overlap_scores = function(df, scalar) {
  ...
  df = df %>%
    mutate(model = factor(model, levels = c("gpt2", "rnng", "jrnn", "grnn", "human"))) %>%
    mutate(score = case_when(
      model == "gpt2" ~ score * 14 * scalar,
      model == "rnng" ~ score * 23 * scalar,
      model == "grnn" ~ score * 10 * scalar,
...
d_avg = d_model %>% filter(l_maze == T) %>% drop_na() %>%
  group_by(group, word_number, condition, suite, critical) %>%
...
d_model_targets = d_avg %>% filter(critical == T) %>% drop_na()

resid_df = data.frame()
resid_critical_result_df = data.frame()
for ( model in c("gpt2", "grnn", "jrnn", "rnng")) {
  f = paste("rt ~",model," + len + freq")
  
  lm = d_avg %>% filter(critical==F) %>% drop_na() %>%
    lm(f, data = .)
...
ggsave("../images/acl/residuals_hist.pdf", width = 4, height = 3, device = cairo_pdf())
...
#ggsave("../images/acl/residuals_vals.pdf", width = 3, height = 3, device = cairo_pdf())
...
resid_pred_df = resid_critical_plot_df %>%
  mutate(resid=abs(resid)) %>%
