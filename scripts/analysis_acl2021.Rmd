...
lm_gpt = d_model %>%
  ...
  mutate(group = as.factor(group), MD5 = as.factor(MD5),
         ...
         len = as.integer(len),
         rt = as.integer(rt)) %>%
  lmer(rt ~ gpt2 + len + freq + (gpt2 + len + freq||MD5) + (gpt2 + len + freq||sentence), data = .,
...
lm_rnng = d_model %>%
  ...
  mutate(group = as.factor(group), MD5 = as.factor(MD5),
         ...
         len = as.integer(len),
         rt = as.integer(rt)) %>%
  lmer(rt ~ rnng + len + freq + (rnng + len + freq||MD5) + (rnng + len + freq||sentence), data = .,
       control=lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))
summary(lm_rnng)
...
lm_grnn = d_model %>%
  ...
  mutate(group = as.factor(group), MD5 = as.factor(MD5),
         ...
         len = as.integer(len),
         rt = as.integer(rt)) %>%
  lmer(rt ~ grnn + len + freq + (grnn + len + freq||MD5) + (grnn + len + freq||sentence), data = .,
       control=lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))
summary(lm_grnn)
...
lm_jrnn = d_model %>%
  ...
  mutate(group = as.factor(group), MD5 = as.factor(MD5),
         ...
         len = as.integer(len),
         rt = as.integer(rt)) %>%
  lmer(rt ~ jrnn + len + freq + (len + freq||MD5) + (len + freq||sentence), data = .,
       control=lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))
summary(lm_jrnn)
